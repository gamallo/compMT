...
threads->create(sub {
    ...
    foreach $key (sort keys %NPV) {
        ($p1, $p2, $p3) = split (/$;/o, $key);
        if ($NPV{$p1,$p2,$p3}>0) {
            printf $fich "CprepV&%s_down_%s %s %d\n", $p2, $p1, $p3 , $NPV{$p1,$p2,$p3};
        #print STDERR "trigram: $p1-$p2-$p3\n";                                                                                                        
            printf $fich "CprepV&%s_up_%s %s %d\n", $p2, $p3, $p1, $NPV{$p1,$p2,$p3};
        }
    ...
    foreach $key (sort keys %VPN) {
        ($p1, $p2, $p3) = split (/$;/o, $key);
        if ($VPN{$p1,$p2,$p3}>0) {
            printf $fich "Iobj&%s_down_%s %s %d\n", $p2, $p1, $p3 , $VPN{$p1,$p2,$p3};
    #print STDERR "trigram: $p1-$p2-$p3\n";                                                                                                       
            printf $fich "Iobj&%s_up_%s %s %d\n", $p2, $p3, $p1, $VPN{$p1,$p2,$p3};
        }
    ...
    foreach $key (sort keys %VPV) {
        ($p1, $p2, $p3) = split (/$;/o, $key);
        if ($VPV{$p1,$p2,$p3}>0) {
            printf $fich "IobjV&%s_down_%s %s %d\n", $p2, $p1, $p3 , $VPV{$p1,$p2,$p3};
    #print STDERR "trigram: $p1-$p2-$p3\n";                                                                                                       
            printf $fich "IobjV&%s_up_%s %s %d\n", $p2, $p3, $p1, $VPV{$p1,$p2,$p3};
        }
...
$file = "temporal_${LING}_".threads->tid();
    open ($fich, ">".$file) or die "O ficheiro temporal non pode ser creado: $!\n";
     foreach $key (sort keys %APN) {
         ($p1, $p2, $p3) = split (/$;/o, $key);
         if ($APN{$p1,$p2,$p3}>0) {
             printf $fich "Aprep&%s_down_%s %s %d\n", $p2, $p1, $p3 , $APN{$p1,$p2,$p3};
       #print STDERR "trigram: $p1-$p2-$p3\n";                                                                                                                                                                                                                                   
             printf $fich "Aprep&%s_up_%s %s %d\n", $p2, $p3, $p1, $APN{$p1,$p2,$p3};
         }
    ...
    foreach $key (sort keys %APV) {
            ($p1, $p2, $p3) = split (/$;/o, $key);
            if ($APV{$p1,$p2,$p3}>0) {
                printf $fich "AprepV&%s_down_%s %s %d\n", $p2, $p1, $p3 , $APV{$p1,$p2,$p3};
        #print STDERR "trigram: $p1-$p2-$p3\n";                                                                                                                                                                                                                                   
                printf $fich "AprepV&%s_up_%s %s %d\n", $p2, $p3, $p1, $APV{$p1,$p2,$p3};
            }
    ...
    ($p1, $p2) = split (/$;/o, $key);
        if ($NN{$p1,$p2}>0) {
        printf $fich "modN_down_%s %s %d\n", $p1, $p2, $NN{$p1,$p2} ;
        # print STDERR "bigram: $p1-modN-$p2\n";
            printf $fich "modN_up_%s %s %d\n", $p2, $p1, $NN{$p1,$p2} ;
        }
    }
    ...
    ($p1, $p2) = split (/$;/o, $key);
        if ($NA{$p1,$p2}>0) {
        printf $fich "Rmod_down_%s %s %d\n", $p1, $p2, $NA{$p1,$p2} ;
        #print STDERR "bigram: $p1-$p2\n";
            printf $fich "Rmod_up_%s %s %d\n", $p2, $p1, $NA{$p1,$p2} ;
        }
...
$file = "temporal_${LING}_".threads->tid();
    ...
    ($p1, $p2) = split (/$;/o, $key);
        if ($AN{$p1,$p2}>0) {
        printf $fich "Lmod_down_%s %s %d\n", $p1, $p2, $AN{$p1,$p2} ;
        #print STDERR "bigram: $p1-$p2\n";
        printf $fich "Lmod_up_%s %s %d\n", $p2, $p1, $AN{$p1,$p2} ;
        }
 ...
 foreach $key (sort keys %RA) {
	 ($p1, $p2) = split (/$;/o, $key);
	 if ($RA{$p1,$p2}>0) {
	     printf $fich "LmodA_down_%s %s %d\n", $p1, $p2, $RA{$p1,$p2} ;
       #print STDERR "bigram: $p1-$p2\n";                                                                                                             
	     printf $fich "LmodA_up_%s %s %d\n", $p2, $p1, $RA{$p1,$p2} ;
	 }
     ...
     foreach $key (sort keys %VR) {
         ($p1, $p2) = split (/$;/o, $key);
         if ($VR{$p1,$p2}>0) {
             printf $fich "RmodV_down_%s %s %d\n", $p1, $p2, $VR{$p1,$p2} ;

             printf $fich "RmodV_up_%s %s %d\n", $p2, $p1, $VR{$p1,$p2} ;
         }
    ...
    foreach $key (sort keys %RV) {
         ($p1, $p2) = split (/$;/o, $key);
         if ($RV{$p1,$p2}>0) {
             printf $fich "LmodV_down_%s %s %d\n", $p1, $p2, $RV{$p1,$p2} ;

             printf $fich "LmodV_up_%s %s %d\n", $p2, $p1, $RV{$p1,$p2} ;
         }
...
$file = "temporal_${LING}_".threads->tid();
    open ($fich, ">".$file) or die "O ficheiro temporal non pode ser creado: $!\n";
     ...
     foreach $key (sort keys %VAmod) {
         ($p1, $p2) = split (/$;/o, $key);
         if ($VAmod{$p1,$p2}>0) {
             printf $fich "Vmod_down_%s %s %d\n", $p1, $p2, $VAmod{$p1,$p2} ;

             printf $fich "Vmod_up_%s %s %d\n", $p2, $p1, $VAmod{$p1,$p2} ;
         }
     ...
     foreach $key (sort keys %VVmod) {
         ($p1, $p2) = split (/$;/o, $key);
         if ($VVmod{$p1,$p2}>0) {
             printf $fich "VmodV_down_%s %s %d\n", $p1, $p2, $VVmod{$p1,$p2} ;

             printf $fich "VmodV_up_%s %s %d\n", $p2, $p1, $VVmod{$p1,$p2} ;
         }
    ...
    ($p1, $p2) = split (/$;/o, $key);
        if ($VN{$p1,$p2}>0) {
        printf $fich "Dobj_down_%s %s %d\n", $p1, $p2,  $VN{$p1,$p2} ;
        #print STDERR "bigram: $p1-$p2\n";
            printf $fich "Dobj_up_%s %s %d\n", $p2, $p1, $VN{$p1,$p2} ;
        }
    ...
    ($p1, $p2) = split (/$;/o, $key);
        if ($NV{$p1,$p2}>0) {
        printf $fich "Subj_down_%s %s %d\n", $p1, $p2, $NV{$p1,$p2} ;
        # print STDERR "bigram: $p1-$p2\n";
            printf $fich "Subj_up_%s %s %d\n", $p2, $p1, $NV{$p1,$p2};
        }
    ...
    ($p1, $p2) = split (/$;/o, $key);
        if ($DVV{$p1,$p2}>0) {
        printf $fich "DobjV_down_%s %s %d\n", $p1, $p2,  $DVV{$p1,$p2} ;
        #print STDERR "bigram: $p1-$p2\n";
            printf $fich "DobjV_up_%s %s %d\n", $p2, $p1, $DVV{$p1,$p2} ;
        }
    ...
    ($p1, $p2) = split (/$;/o, $key);
        if ($SVV{$p1,$p2}>0) {
        printf $fich "SubjV_down_%s %s %d\n", $p1, $p2,  $SVV{$p1,$p2} ;
        #print STDERR "bigram: $p1-$p2\n";
            printf $fich "SubjV_up_%s %s %d\n", $p2, $p1, $SVV{$p1,$p2} ;
        }
    }
